...
fasi_err_full <- function(test_data, method) {
  ...
  error_rates <- sapply(all_groups, function(class_cur) {
    ...
    err_full <- ifelse(nrow(x)==0, 0, mean((x$true_label != class_cur)))
...
aci_rank <- function(train_data, test_data, alpha_param, method) {
  ...
  rscore_list <- lapply(c(0,1), function(pt_c) {
    ...
    #   return(rscore_fcn(test_cur, class_2_score_train_r1, class_2_score_test_r1, train_df$true_label, T))
    # })
...
num_dec_allgroup <- function(dta, method) {
  ...
  info <- pull(dta[method_loc])
  num_dec <- table(factor(info, levels = 0:2))

  num_dec_indecisions <- num_dec[1]
  return(num_dec_indecisions)
}

fasi_fcn_logit <- function(train_data, test_data, naive_indicator=T, n_iter) {
  label_train <- train_data$true_label
  label_test <- test_data$true_label
  ...
  class_scores <- sapply(1:2, function(ii) {
    ...
    if (naive_indicator==T) {
      # gam_formula <- as.formula("y ~ s(x1,3) + s(x2,3) + s(x3,3)")
      logit_formula <- as.formula("y ~ x1 + x2 + x3")
    } else {
      # gam_formula <- as.formula("y ~ s(x1,3) + s(x2,3) + s(x3,3) + protected_class")
      logit_formula <- as.formula("y ~ x1 + x2 + x3 + protected_class")
    }
    model_logit <- glm(logit_formula, data = dta_gam, family = "binomial")
    score_2 <- predict(model_logit, newdata = dta_test, type = "response")
...
fasi_fcn_gam <- function(train_data, test_data, naive_indicator=T, n_iter) {
  label_train <- train_data$true_label
  label_test <- test_data$true_label
...
fasi_fcn_xgboost <- function(train_data, test_data, naive_indicator=T, n_iter) {
  label_train <- train_data$true_label
  ...
  class_scores <- sapply(1:2, function(ii) {
    ...
    xg_boost_model <- xgboost(data = data.matrix(dta_train), label = (as.numeric(y_train)-1),
                              objective = "binary:logistic", nrounds = 10)
    score_2 <- predict(xg_boost_model, data.matrix(dta_test))
...
fasi_fcn_npnb <- function(train_data, test_data, naive_indicator=T, n_iter) {
  label_train <- train_data$true_label
  ...
  model <- nonparametric_naive_bayes(x = data.matrix(dta_train), y = as.factor(y_train))
  score_2 <- predict(model, newdata = data.matrix(dta_test), type = "prob")
  class_scores <- as.matrix(score_2)
...
fasi_fcn_nb <- function(train_data, test_data, naive_indicator=T, n_iter) {
  label_train <- train_data$true_label

  class_scores <- lapply(1:1, function(ii) {
    if (ii == 1) {
      ## Change all 2's to 1
      label_train[train_data$true_label == 1] <- 2
      label_train[train_data$true_label == 2] <- 1
    ...
    ## Subset the train / test data on the vars we need to model
    dta_train_model <- train_data %>%
      dplyr::select(all_of(keep_vars))
    dta_test_model <- test_data %>%
      dplyr::select(all_of(keep_vars))

    dta_svm <- cbind.data.frame(y=as.factor(as.numeric(y_train)-1), dta_train_model)
    model <- svm(y~., data = dta_svm, probability = TRUE, type="C")
    predictions <- predict(model, newdata = dta_test, probability = TRUE)
