...
avg_matrix <- matrix(nrow=length(p_iter), ncol=28)
sd_matrix <- matrix(nrow=length(p_iter), ncol=28)
p_iter_fullrun <- list()
...
for (i in 1:length(p_iter)) {
  ...
  diagnostics_list <- mclapply(1:nits, function(dummy_iter) {
    ...
    ## Num Indecisions
    logit_epi <- num_dec_allgroup(dta_test, method = "logit_class") / nrow(dta_test)
    names(logit_epi) <- paste("Logit", names(logit_epi))

    gam_epi <- num_dec_allgroup(dta_test, method = "gam_class") / nrow(dta_test)
    names(gam_epi) <- paste("GAM", names(gam_epi))

    xg_epi <- num_dec_allgroup(dta_test, method = "xg_class") / nrow(dta_test)
    names(xg_epi) <- paste("XG Boost", names(xg_epi))

    nb_epi <- num_dec_allgroup(dta_test, method = "nb_class") / nrow(dta_test)
...
avg_mat_logit <- cbind.data.frame(EPI=avg_matrix[,grep("Logit 0", colnames(avg_matrix))], p_iter)
epi_logit_plt_dta <- avg_mat_logit
...
avg_mat_gam <- cbind.data.frame(EPI=avg_matrix[,grep("GAM 0", colnames(avg_matrix))], p_iter)
epi_gam_plt_dta <- avg_mat_gam
...
avg_mat_xg <- cbind.data.frame(EPI=avg_matrix[,grep("XG Boost 0", colnames(avg_matrix))], p_iter)
epi_xg_plt_dta <- avg_mat_xg
...
### SVM Boost Plot
avg_mat_svm <- avg_matrix[,grep("NB", colnames(avg_matrix))]
avg_mat_svm_fsr <- avg_mat_svm[,grep("FSR", colnames(avg_mat_svm))]
avg_mat_svm_fsr_c2 <- cbind.data.frame(avg_mat_svm_fsr[,grep("Class 2", colnames(avg_mat_svm_fsr))], p_iter)
fsr_svm_plt_dta <- gather(avg_mat_svm_fsr_c2, protected_group, FSR, "NB FSR Protected Class Full Class 2":"NB FSR Protected Class 1 Class 2", factor_key=TRUE)
protected_group <- c()
protected_group[grep("Protected Class 1", fsr_svm_plt_dta$protected_group)] <- "Male"
protected_group[grep("Protected Class 0", fsr_svm_plt_dta$protected_group)] <- "Female"
protected_group[grep("Protected Class Full", fsr_svm_plt_dta$protected_group)] <- "Female and Male"
fsr_svm_plt_dta$protected_group <- protected_group

svm_fsr <- ggplot(fsr_svm_plt_dta, aes(x=p_iter, y=FSR)) +
  geom_line(aes(color = protected_group, linetype = protected_group)) +
...
avg_mat_svm <- cbind.data.frame(EPI=avg_matrix[,grep("NB 0", colnames(avg_matrix))], p_iter)
epi_svm_plt_dta <- avg_mat_svm

svm_epi <- ggplot(epi_svm_plt_dta, aes(x=p_iter, y=EPI)) +
  geom_line(color='#009E73') +
  theme_minimal(base_size = 7) +
  xlab("$\\pi_{2,F}$") +
  ylab("EPI") +
