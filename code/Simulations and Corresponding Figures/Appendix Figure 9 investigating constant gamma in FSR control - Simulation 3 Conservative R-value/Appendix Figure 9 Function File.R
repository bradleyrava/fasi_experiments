...
aci_rank <- function(train_data, test_data, alpha_param, method) {
  ...
  rscore_list <- lapply(c(0,1), function(pt_c) {
    ...
    r1_score <- sapply(class_2_score_test_r1, function(test_cur) {
      return(rscore_fcn(test_cur, class_2_score_train_r1, class_2_score_test_r1, train_df$true_label, pt_c, train_df$protected_class, test_df$protected_class, T, F))
    })


    r2_score <- sapply(class_2_score_test_r2, function(test_cur) {
      return(rscore_fcn(test_cur, class_2_score_train_r2, class_2_score_test_r2, train_df$true_label, pt_c, train_df$protected_class, test_df$protected_class, F, F))
    })

    r1_score_conserv <- sapply(class_2_score_test_r1, function(test_cur) {
      return(rscore_fcn(test_cur, class_2_score_train_r1, class_2_score_test_r1, train_df$true_label, pt_c, train_df$protected_class, test_df$protected_class, T, T))
    })

    r2_score_conserv <- sapply(class_2_score_test_r2, function(test_cur) {
      return(rscore_fcn(test_cur, class_2_score_train_r2, class_2_score_test_r2, train_df$true_label, pt_c, train_df$protected_class, test_df$protected_class, F, T))
    })

    return_scores <- cbind.data.frame(r1_score, r2_score, r1_score_conserv, r2_score_conserv, index_rows)
  ...
  classification_vec_conserv <- vector(mode="numeric", length = nrow(r_scores_order))
  classification_vec_conserv[which(r_scores_order$r1_score_conserv <= alpha_param)] <- 1
  classification_vec_conserv[which(r_scores_order$r2_score_conserv <= alpha_param)] <- 2

  return(list(classification_vec, classification_vec_conserv))
}
